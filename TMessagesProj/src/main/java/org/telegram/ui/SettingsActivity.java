...
//        fragmentView.addView(navigationBar, LayoutHelper.createFrame(LayoutHelper.MATCH_PARENT, 0, Gravity.BOTTOM | Gravity.FILL_HORIZONTAL));
    ...
    private void fillItems(ArrayList<UItem> items, UniversalAdapter adapter) {
        ...
        items.add(UItem.asSwitchNoIcon(26, "شبح مشاهدة الستوريات").setChecked(ExteraFeatures.isGhostStoriesEnabled(currentAccount)));
        items.add(UItem.asSwitchNoIcon(27, "إخفاء الإعلانات والرعايات").setChecked(ExteraFeatures.isHideAdsEnabled(currentAccount)));
        items.add(UItem.asSwitchNoIcon(28, "السماح بحفظ/تحويل المحتوى المقيد").setChecked(ExteraFeatures.isProtectedContentEnabled(currentAccount)));
        items.add(UItem.asSwitchNoIcon(29, "حفظ الصور والفيديوهات ذاتية الحذف").setChecked(ExteraFeatures.isSaveSelfDestructEnabled(currentAccount)));
        items.add(UItem.asSwitchNoIcon(30, "كتم كل المحادثات").setChecked(ExteraFeatures.isMuteAllEnabled(currentAccount)));
        items.add(UItem.asSwitchNoIcon(31, "إظهار أعضاء المجموعات المخفيين").setChecked(ExteraFeatures.isShowHiddenMembersEnabled(currentAccount)));
        items.add(UItem.asShadow(null));
    ...
    private void onClick(UItem item, View view, int position, float x, float y) {
        ...
        switch (item.id) {
            ...
            case 26: {
                boolean enabled = !ExteraFeatures.isGhostStoriesEnabled(currentAccount);
                ExteraFeatures.setGhostStoriesEnabled(currentAccount, enabled);
                item.setChecked(enabled);
                listView.adapter.update(true);
                break;
            }
            case 27: {
                boolean enabled = !ExteraFeatures.isHideAdsEnabled(currentAccount);
                ExteraFeatures.setHideAdsEnabled(currentAccount, enabled);
                item.setChecked(enabled);
                listView.adapter.update(true);
                break;
            }
            case 28: {
                boolean enabled = !ExteraFeatures.isProtectedContentEnabled(currentAccount);
                ExteraFeatures.setProtectedContentEnabled(currentAccount, enabled);
                item.setChecked(enabled);
                listView.adapter.update(true);
                break;
            }
            case 29: {
                boolean enabled = !ExteraFeatures.isSaveSelfDestructEnabled(currentAccount);
                ExteraFeatures.setSaveSelfDestructEnabled(currentAccount, enabled);
                item.setChecked(enabled);
                listView.adapter.update(true);
                break;
            }
            case 30: {
                boolean enabled = !ExteraFeatures.isMuteAllEnabled(currentAccount);
                ExteraFeatures.setMuteAllEnabled(currentAccount, enabled);
                item.setChecked(enabled);
                listView.adapter.update(true);
                break;
            }
            case 31: {
                boolean enabled = !ExteraFeatures.isShowHiddenMembersEnabled(currentAccount);
                ExteraFeatures.setShowHiddenMembersEnabled(currentAccount, enabled);
                item.setChecked(enabled);
                listView.adapter.update(true);
                break;
            }
