...
//            }
    ...
    private ActionBarMenuSubItem actionContinuousButton;
    private final android.os.Handler exteraRepeatHandler = new android.os.Handler(android.os.Looper.getMainLooper());
    private Runnable exteraRepeatRunnable;
    private void openExteraTemporarySendMenu() {
        if (messageEditText == null || TextUtils.isEmpty(messageEditText.getText()) || parentFragment == null) return;
        final String text = messageEditText.getText().toString();
        final long targetDialog = parentFragment.getDialogId();
        CharSequence[] choices = new CharSequence[]{"كل دقيقة", "كل دقيقتين", "كل ساعة", "كل يوم"};
        new AlertDialog.Builder(getContext())
                .setTitle("إرسال مؤقت")
                .setItems(choices, (dialog, which) -> {
                    int period = which == 0 ? 60 : which == 1 ? 120 : which == 2 ? 3600 : 86400;
                    AlertsCreator.createScheduleDatePickerDialog(parentActivity, targetDialog, (notify, scheduleDate, ignoredRepeatPeriod) -> {
                        SendMessagesHelper.SendMessageParams params = SendMessagesHelper.SendMessageParams.of(text, targetDialog);
                        params.notify = notify;
                        params.scheduleDate = scheduleDate;
                        params.scheduleRepeatPeriod = period;
                        SendMessagesHelper.getInstance(currentAccount).sendMessage(params);
                        ExteraFeatures.setTemporarySend(currentAccount, true, period, 0);
                    }, resourcesProvider);
                }).show();
    }

    private void openExteraContinuousMenu() {
        if (messageEditText == null || TextUtils.isEmpty(messageEditText.getText()) || parentFragment == null) return;
        final String text = messageEditText.getText().toString();
        final long targetDialog = parentFragment.getDialogId();
        CharSequence[] choices = new CharSequence[]{"دقيقة", "دقيقتين", "3 دقائق", "4 دقائق", "5 دقائق", "10 دقائق", "ساعة", "يوم", "إيقاف"};
        new AlertDialog.Builder(getContext())
            .setTitle("نشر متواصل")
            .setItems(choices, (dialog, which) -> {
                dialog.dismiss();
                ...
                long period = (which == 0 ? 60 : which == 1 ? 120 : which == 2 ? 180 : which == 3 ? 240 : which == 4 ? 300 : which == 5 ? 600 : which == 6 ? 3600 : 86400) * 1000L;
                startExteraContinuous(text, targetDialog, period);
            }).show();
    }

    private void startExteraContinuous(String text, long dialogId, long periodMs) {
        stopExteraContinuous();
        ExteraFeatures.setRepeatPostingEnabled(currentAccount, true);
        exteraRepeatRunnable = new Runnable() {
        ...
        exteraRepeatHandler.post(exteraRepeatRunnable);
    }

    private void stopExteraContinuous() {
        ExteraFeatures.setRepeatPostingEnabled(currentAccount, false);
        if (exteraRepeatRunnable != null) {
            exteraRepeatHandler.removeCallbacks(exteraRepeatRunnable);
            exteraRepeatRunnable = null;
        }
    }
        ...
        if (isStories || (messageEditText == null || TextUtils.isEmpty(messageEditText.getText())) && parentFragment != null && parentFragment.messagePreviewParams != null && parentFragment.messagePreviewParams.forwardMessages != null && parentFragment.messagePreviewParams.forwardMessages.messages != null && !parentFragment.messagePreviewParams.forwardMessages.messages.isEmpty()) {
            ...
            if (sendPopupLayout == null) {
                ...
                if (scheduleButtonValue) {
                    ...
                    actionContinuousButton.setTextAndIcon(ExteraFeatures.isRepeatPostingEnabled(currentAccount) ? "تعطيل الإرسال المؤقت" : "إرسال مؤقت", R.drawable.msg_calendar);
                    ...
                    actionContinuousButton.setOnClickListener(v -> {
                        ...
                        if (ExteraFeatures.isRepeatPostingEnabled(currentAccount)) {
                            stopExteraContinuous();
                        } else {
                            openExteraTemporarySendMenu();
                        }
                    });
                    sendPopupLayout.addView(actionContinuousButton, LayoutHelper.createLinear(LayoutHelper.MATCH_PARENT, DEFAULT_HEIGHT));
